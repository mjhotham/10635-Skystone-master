...
// @Autonomous
public class Blue3SkystoneAuto extends LinearOpMode {     // Experimental Auto
...
//    DriveConstraints  = new DriveConstraints(30, 30, 0, Math.PI, Math.PI, 0);
    ...
    Pose2d quickDeposit2 = new Pose2d(quickDeposit.getX(), quickDeposit.getY() + 4, quickDeposit.getHeading());
    ...
    public void runOpMode() throws InterruptedException {
        ...
        Pose2d thirdStonePosition;
        switch (pos) {
            case Left:
                thirdStonePosition = secondCenterPickup;
                break;
            case Center:
                thirdStonePosition = secondRightPickup;
                break;
            case Right:
                thirdStonePosition = secondLeftPickup;
                break;
            default:
                thirdStonePosition = secondRightPickup;
                break;
        ...
        drive.followTrajectory(drive.trajectoryBuilder().reverse().splineTo(underBridge).splineTo(quickDeposit2).build());//all in reverse

        lift.slideTargetIN = RobotConstants.FirstDepositTopSlideTarget;

        lift.start(RobotConstants.FirstDepositHeight);
        //deposit first(second) skystone
        while (lift.isBusy || drive.isBusy()) {
            if (drive.getPoseEstimate().getY() < underBridge.getY()) {//only run code after passed bridge
                RevBulkData bulkData = drive.hub2.getBulkInputData();
                drive.LeftAngle.setPosition(RobotConstants.LeftAngleOpenAuto);
        ...
        lift.liftTargetIN = 0;
        lift.slideTargetIN = 0;
        drive.followTrajectory(drive.trajectoryBuilder().splineTo(underBridge).splineTo(thirdStonePosition).forward(8).build());
        while (drive.isBusy()) {
            RevBulkData bulkData = drive.hub2.getBulkInputData();
            if (bulkData != null) {
                lift.update(bulkData);
        ...
        lift.slideTargetIN = RobotConstants.SecondDepositTopSlideTarget + 4;  // the number you are looking for is 14
        ...
        while (lift.isBusy || drive.isBusy()) {
            if (drive.getPoseEstimate().getY() < underBridge.getY()) {//only run code after passed bridge
                ...
                if (bulkData != null) {
                    lift.update(bulkData);//updates lift and slide checking for collisions
                    if (lift.SlidePositionIN > 12)
                        drive.Wrist.setPosition(RobotConstants.WristRightDepositPosition);
                }
            }
            drive.update();
            telemetry.update();
