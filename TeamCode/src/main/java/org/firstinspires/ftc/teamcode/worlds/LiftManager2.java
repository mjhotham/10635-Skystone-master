...
public class LiftManager2 {
    ...
    public LiftManager2(ExpansionHubMotor LeftLift, ExpansionHubMotor RightLift, ExpansionHubServo Elbow, ExpansionHubServo Elbow2, ExpansionHubMotor slideEncoder) {
...
//                liftOffset = (bulkData2.getMotorCurrentPosition(LeftLift) - bulkData2.getMotorCurrentPosition(RightLift)) / (LeftLift.getMotorType().getTicksPerRev());
        ...
        } else if (Math.abs(slideTargetIN - SlidePositionIN) < 1 && smoothnessTimerSlide.milliseconds() < 1000) {
            slideTargetIN = SlidePositionIN;
            Elbow.setPosition(0.5);
            Elbow2.setPosition(0.5);
        ...
        if (slideObstruction) {
            ...
            isBusy = Math.abs(SlidePositionIN - slideTargetIN) > 1;
        }
    }
