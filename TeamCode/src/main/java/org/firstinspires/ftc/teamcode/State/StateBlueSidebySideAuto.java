...
@Autonomous(name = "Blue 2x2 Base Auto")
public class StateBlueSidebySideAuto extends LinearOpMode {
    ...
    Pose2d firstDeposit = new Pose2d(37, -48.6, .45);
    ...
    public void runOpMode() throws InterruptedException {
        ...
        double myBestEstimateForNow = .694;
        ...
        while (lift.isBusy || drive.isBusy()) {
            if (drive.getPoseEstimate().getY() < underBridge.getY()) {//only run code after passed bridge
                ...
                if (lift.SlidePositionIN > 10)//10? need speed right?
                    drive.Wrist.setPosition(myBestEstimateForNow);
            }
            drive.update();
        }
